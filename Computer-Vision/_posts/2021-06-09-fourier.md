...
<span class="display-6">Similarly</span> to my previous [post]({% post_url Computer-Vision/2021-06-08-vectorization %}),
...
{% include figure.html
   ...
   figcaption="Figure 1: Samples in the TF-Flowers dataset." %}
...
{% include figure.html
   ...
   figcaption="Figure 2: Illustration of the Fourier and Inverse Fourier Transformations over 2D signals (images). From top to bottom: (a) the original images; (b) the (log of) spectrum of the Fourier transform; (c) the spectrum shifted; (d) the un-shifted spectrum; and (e) the (real) result of the Inverse Fourier Transform."
   classed="w-xl-130" %}
...
{% include figure.html
   ...
   figcaption="Figure 4: Illustration of steps involved in applying a low-pass filter in the frequency domain using the Fast Fourier Fourier Transformation over 2D signals (images). From top to bottom: (a) the original images; (b) the (log of) spectrum of the Fourier transform; (c) the spectrum shifted; (d) the (log of) spectrum multiplied by the ideal circular low-pass filter; (e) the un-shifted spectrum; and (f) the (real) result of the Inverse Fourier Transform."
   classed="w-xl-130" %}
...
{% include figure.html
   ...
   figcaption="Figure 5: Result of the circular ideal filter in the frequency domain, varying its radius. The following radii values were adopted (top to bottom): 5, 10, 20, 40, 70 and 150."
   classed="w-xl-130" %}
...
{% include figure.html
   ...
   figcaption="Figure 6: Result of the Butterworth low-pass filter in the frequency domain, varying its radius. The following radii values were adopted (top to bottom): 1, 2%, 3%, 5%, 10% 50%."
   classed="w-xl-130" %}
...
{% include figure.html
   ...
   figcaption="Figure 7: Result of the ideal high-pass filter in the frequency domain, varying its radius. The following radii values were adopted (top to bottom): 1, 2, 3, 10, 40 and 70."
   classed="w-xl-130" %}
...
{% include figure.html
   ...
   figcaption="Figure 8: Result of the ideal band-pass filter in the frequency domain, varying its radius. The following (inner, outer) radii values were adopted (top to bottom): (1, 3), (5, 10), (10, 15), (10, 50), (30, 90), (40, 140), (70, 140), (30, 150) and (5, 150)."
   classed="w-xl-130" %}
...
{% include figure.html
   ...
   figcaption="Figure 10: Effect of rotation in the spectrum of the Fourier Transformation of a signal."
   classed="w-xl-130" %}
...
{% include figure.html
   ...
   figcaption="Figure 9: Result of the compression filter in the frequency domain, varying its rate. The following rate values were adopted (top to bottom): 10%, 50%, 90%, 95%, 99%, and 99.9%."
   classed="w-xl-130" %}
